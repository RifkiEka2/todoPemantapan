...
<body class="bg-gray-200 w-3/6 flex flex-col items-center mx-auto h-screen">
    <div class="container h-44 mt-10 shadow-lg flex flex-col items-center justify-center bg-white p-6 rounded-lg">
        ...
        <p class="mb-4">Dibuat Pada: {{ $task->created_at->format('d-m-Y') }}</p>
        <button onclick="document.getElementById('modal').classList.remove('hidden')" class="bg-zinc-900 py-2 px-6 rounded-full text-white font-bold transition-transform duration-00 hover:scale-105">Buat SubTugas</button>
    </div>

    <div class="bg-white shadow-lg w-full py-5 mt-10 flex flex-col px-8 pt-4 rounded-lg">
        <div class="flex justify-between items-center border-b-2 border-zinc-900 pb-2 mb-4">
            ...
            <div class="flex items-center gap-7">
                <form action="{{ route('subtasks.toggleStatus', $subtask->id) }}" method="POST">
                    ...
                    <input type="checkbox" name="status" 
                        ...
                        onchange="this.form.submit()" /> <!-- Submit form saat checkbox diubah -->
                </form>
                
                <div class="flex flex-col justify-center rounded-lg ">
                    <h2 class="font-bold text-2xl text-white {{ $subtask->status === 'done' ? 'line-through text-gray-600' : '' }}">
                        {{ $subtask->title }}
                    </h2>
                    ...
                    @php
                    $warnaPrioritas = $subtask->status === 'done' ? 'bg-gray-600 text-zinc-900'
                ...
                <div class="{{ $warnaPrioritas }} w-24 mt-2 rounded-full flex justify-center items-center text-white">
                    <p class=" font-semibold">{{ $subtask->priority }}</p>
                </div>
                </div>
            </div>
            <div class="gap-2 flex ">
                <button onclick="openSubtaskEditModal('{{ $subtask->id }}', '{{ $subtask->title }}', '{{ $subtask->deadline }}', '{{ $subtask->priority }}')"
                    class="bg-white text-zinc-900 font-bold py-2 px-6 rounded-md">
                    Edit
                </button>
                <button  onclick="openDeleteModal('{{ $task->id }}', '{{ $task->title }}')" class="bg-white text-zinc-900 font-bold py-2 px-4 rounded-md">Hapus</button>
            </div>
        </div>
        @endforeach
    </div>
        ...
        <div id="modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden">
            <div class="bg-white rounded-lg p-6 w-full max-w-md ">
                ...
                <form action="{{ route('subtasks.store', ['task' => $task->id]) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="title" required class="mt-1 block w-full border border-gray-300 rounded-md p-2" placeholder="Masukkan judul">
                    </div>

                    <div class="mb-4">
                        <label class="block text-md font-bold ">Tenggat Waktu</label>
                        <input type="date" name="deadline" required class="mt-1 block w-full border border-gray-300 rounded-md p-2" placeholder="Masukkan judul">
                    </div>

                    <div class="mb-4">
                        <label class="block text-md font-bold ">Prioritas</label>
                        <select name="priority" name="priority" class="mt-1 block w-full border border-gray-300 rounded-md p-2">
                    ...
                    <div class="flex justify-end gap-2">
                        <button type="button" onclick="document.getElementById('modal').classList.add('hidden')" class="text-white bg-zinc-900 font-bold py-2 px-5 rounded-md">Batal</button>
            ...
            <!-- Modal Edit -->
            <div id="subtaskEditModal" class="fixed inset-0 z-50 bg-gray-600 bg-opacity-50 flex justify-center items-center hidden">
                <div class="bg-white rounded-lg p-6 w-full max-w-md">
                    <div class="flex justify-center">
                    ...
                    <form id="subtaskEditForm" method="POST" action="{{ route('subtasks.update', $task->id) }}">
                        ...
                        <div class="mt-4">
                            ...
                            <input type="text" name="title" id="subtaskEditTitle" class="w-full p-2 border border-gray-300 rounded" required>
                        </div>
            
                        <div class="mt-4">
                            <label class="block font-bold">Tenggat Waktu</label>
                            <input type="date" name="deadline" id="subtaskEditDeadline" class="w-full p-2 border border-gray-300 rounded" required>
                        </div>
            
                        <div class="mt-4">
                            <label class="block font-bold">Prioritas</label>
                            <select name="priority" id="subtaskEditPriority" class="w-full p-2 border border-gray-300 rounded" required>
                                <option value="rendah">Rendah</option>
                                <option value="sedang">Sedang</option>
                                <option value="tinggi">Tinggi</option>
                            </select>
                        ...
                        <div class="mt-6 flex justify-end gap-3">
                            <button type="button" onclick="closeSubtaskEditModal()" class="text-white bg-zinc-900 font-bold py-2 px-5 rounded-md">Batal</button>
                            <button class="text-white bg-zinc-900 font-bold py-2 px-4 rounded-md">Simpan</button>
                        </div>
                    </form>
                </div>
            ...
            <div id="deleteModal" class="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50 hidden">
                <div class="bg-white rounded-lg p-6 w-full max-w-md">
                    ...
                    <div class="flex justify-center space-x-2">
                        <button type="button" onclick="closeDeleteModal()" class="text-white bg-zinc-900 font-bold py-2 px-5 rounded-md">Batal</button>
                        <form id="deleteForm" action="{{ route('subtasks.destroy', $task->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
