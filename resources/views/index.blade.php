...
<body class="bg-gray-200 w-3/6 flex flex-col items-center mx-auto h-screen">
        ...
        <div class="flex items-center w-full justify-center h-screen">
            <div class="w-full shadow-lg flex flex-col items-center justify-center bg-white p-6 rounded-lg">
                <h1 class="text-3xl font-bold">Welcome to Todo App</h1>
                <p class="mb-4"></p>
                <button onclick="document.getElementById('modal').classList.remove('hidden')" class="bg-zinc-900 py-2 px-6 rounded-full text-white font-bold transition-transform duration-200 hover:scale-105">
                    Buat Mata Pelajaran
                </button>
            </div>
        </div>
        @else
        <div class="sticky top-10 z-10 container h-44 mt-10 shadow-lg flex flex-col items-center justify-center bg-white p-6 rounded-lg">
            <h1 class="text-3xl font-bold ">Welcome to Todo App</h1>
            <p class="mb-4"></p>
            <button onclick="document.getElementById('modal').classList.remove('hidden')" class="bg-zinc-900 py-2 px-6 rounded-full text-white font-bold transition-transform duration-200 hover:scale-105">
                Buat Mata Pelajaran
        ...
        <div class="bg-white shadow-lg w-full py-5 mt-10 flex flex-col px-8 pt-4  rounded-lg">
            <div class="flex justify-between items-center mb-4 border-b-2 border-zinc-900 pb-2">
                <h2 class="font-bold text-2xl">Daftar Mata Pelajaran</h2>
            </div>


        @foreach ($tasks as $task)
        <div class="relative bg-zinc-800 hover:bg-zinc-900 h-32 rounded-lg flex justify-between items-center pl-10 mb-4 transition-transform duration-500 hover:scale-105">
            ...
            <div class="absolute right-10 top-1/2 -translate-y-1/2 gap-2 flex ">
                <button onclick="openEditModal('{{ $task->id }}', '{{ $task->title }}')" class="bg-white text-zinc-900 font-bold py-2 px-6 rounded-md">Edit</button>
    ...
    <div class="bg-white rounded-lg p-6 w-full max-w-md">
        ...
        <form id="editForm" action="{{ route('tasks.update', 'task_id') }}" method="POST">
            ...
            <div class="mt-4">
                ...
                <input type="text" name="title" id="editTitle" class="w-full p-2 border border-gray-300 rounded" value="" required>
            </div>

            <div class="mt-4 flex justify-end gap-3">
                <button type="button" onclick="closeModal()" class="text-white bg-zinc-900 font-bold py-2 px-5 rounded-md">Kembali</button>
                <button class="text-white bg-zinc-900 font-bold py-2 px-4 rounded-md">Simpan</button>
            </div>
        </form>
    </div>
